...
class ReportExtraContent(models.Model):
    ...
    model_id = fields.Many2one('ir.model', string="Model", required=True, domain=[(
        'model', 'in', ['account.invoice', 'sale.order', 'stock.picking', 'purchase.order'])])
    company_id = fields.Many2one('res.company', string='Company', change_default=True,
                                 required=True, default=lambda self: self.env['res.company']._company_default_get('report.extra.content'))

    _sql_constraints = [
        ('unique_model_company', 'unique (model_id, company_id)', 'Model and Company must be unique.')
    ]
    ...
    def convert_dxp2pdf(self, values):
        ...
        if fname:
            ...
            output = subprocess.check_output(['libreoffice', '--convert-to', 'pdf', dxp_file_path])
