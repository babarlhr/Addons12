...
class ResCompany(models.Model):
    ...
    font = fields.Many2one('report.fonts', string="Report Font", default=lambda self: self._get_font(),
    ...
    font_size = fields.Integer('Report Font Size(px)', default=12, required=True)
    ...
    def action_view_report_extra_content(self):
        action = self.env.ref('general_template.action_report_extracontent').read()[0]
    ...
    def _compute_logo_footer(self):
        for company in self:
            company.logo_footer = tools.image_resize_image_medium(company.partner_id.image)
